...
const vector<vector<int>> Mario::mario_sprite_normal_2 = {
...
void Mario::paint2(pro2::Window& window) const {
    const Pt top_left = {pos_.x - 6, pos_.y - 15};
    paint_sprite(window, top_left, mario_sprite_normal_2, looking_left_);
}
...
void Mario::update(pro2::Window& window, const vector<Platform>& platforms) {
    last_pos_ = pos_;
    if (window.is_key_down(jumpkey_)) {
        jump();
    }
    ...
    if (window.is_key_down(left_key)) {
        speed_.x = -4;
    } else if (window.is_key_down(right_key)) {
        speed_.x = 4;
    }
    if (speed_.x != 0) {
